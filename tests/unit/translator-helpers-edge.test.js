...
describe("normalizeClaudePassthrough — haiku adaptive thinking (docs 11 §1)", () => {
  ...
  it("folds mid-conversation system messages into the neighbouring user turn", () => {
    // Anthropic rejects a mid-conversation `system` role, and hoisting it into
    // body.system moved the cached prefix. It is folded into the adjacent user
    // turn instead (v0.5.55), so no `system` role survives and no top-level
    // `system` is synthesised.
