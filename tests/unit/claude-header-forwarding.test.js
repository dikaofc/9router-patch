...
// ─── proxyFetch routing ───────────────────────────────────────────────────────
// TLS fingerprint spoofing via got-scraping was removed from proxyFetch.js (it
// was never a dependency and was already disabled) — every host, including
// api.anthropic.com, now goes through native fetch. These tests pin that.

describe("proxyAwareFetch — host routing", () => {
  ...
  it("never loads got-scraping (module removed from the dependency tree)", async () => {
    const factory = vi.fn(() => ({ gotScraping: vi.fn() }));
    vi.doMock("got-scraping", factory);
    stubFetch();
    ...
    // The mocked module is only instantiated if proxyFetch imports it.
    expect(factory).not.toHaveBeenCalled();
  });

  it("routes non-Anthropic hosts through native fetch too", async () => {
    const stub = stubFetch();
